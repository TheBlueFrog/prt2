...
//    INSERT INTO "MessageData" VALUES(null,'raw msg','parsed msg');
    ...
    /**
     * insert a new message from the bot to the user into the DB
     *
     * @param src
     * @param dst       destination use GUID
     * @param msg       the message
     ...
     * @throws SQLException
     */
    public boolean insertNewMsg(String src, String dst, String msg) throws SQLException {
        PreparedStatement s = null;
        PreparedStatement s1 = null;
        try {
            String q = String.format("insert into MessageData values (null, \"%s\", \"%s\")", msg, "");
            s = mDB.prepareStatement(q);
            boolean b = s.execute();
            long rowid = s.getGeneratedKeys().getInt(1);
     ...
     */
    public long insertConsumerListing(long consumerID, ListedItem item) throws SQLException {
        return _insertListing("ConsumerListingData", consumerID, item);
    }
    public long insertSupplierListing(long supplierID, ListedItem item) throws SQLException {
        return _insertListing("SupplierListingData", supplierID, item);
    }
    public long _insertListing(String table, long owner, ListedItem item) throws SQLException {
        PreparedStatement s = null;
        PreparedStatement s1 = null;
        try {
            long rowid = findOrInsertItem(item.getText());

            long now = System.currentTimeMillis();

            String q1 = String.format(
                    "insert into %s values (null, %d, %d, %d, %d, %f, \"%s\", %d)",
                    table, now, owner,
