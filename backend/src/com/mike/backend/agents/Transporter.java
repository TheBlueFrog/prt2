...
public class Transporter extends LocatedAgent {
    ...
    private Map<Consumer, List<Consumable>> pickups = new HashMap<>();
    private Map<Consumer, List<Consumable>> deliveries = new HashMap<>();
...
//                c = Color.gray;
    ...
    private int countPickups() {
        ...
        for (Consumer consumer : pickups.keySet())
            for (Consumable c : pickups.get(consumer))
                i++;
        return i;
    }
    private int countDeliveries() {
        ...
        for (Consumer consumer : deliveries.keySet())
            for (Consumable c : deliveries.get(consumer))
                i++;
        return i;
    }


    public Transporter(Framework f, Integer serialNumber) {
...
//                        tb.getTag()));
    ...
    private Map<Consumer,List<Consumable>> listToMap(List<Consumable> consumables) {
        Map<Consumer,List<Consumable>> m = new HashMap<>();
        for (Consumable c : consumables) {
            if ( ! m.containsKey(c.getConsumer()))
                m.put(c.getConsumer(), new ArrayList<Consumable>());
    ...
    private List<Supplier> getSuppliersToVisit() {
        ...
        for (Consumer consumer : pickups.keySet())
            for (Consumable consumable : pickups.get(consumer)) {
...
//        Log.d(TAG, String.format("%3d Remaing distance %f", this.getSerialNumber(), metersToDest));
    ...
    private boolean pickup(Task task)
    {
        for (Consumer consumer : pickups.keySet()) {
            ...
            List<Consumable> v = new ArrayList<>(pickups.get(consumer));
            for (Consumable consumable : v) {
...
//                    Log.d(TAG, String.format("%d remove %s from pickups", getSerialNumber(), consumable.toString()));
    ...
    private void addDelivery(Consumer consumer, Consumable consumable) {
        if ( ! deliveries.containsKey(consumer))
            deliveries.put(consumer, new ArrayList<Consumable>());
...
//                task.consumer.getSerialNumber()));
        ...
        for (Consumer consumer : deliveries.keySet()) {
            // make copy of the list we're going to remove from and iterate that
            List<Consumable> v = new ArrayList<>(deliveries.get(consumer));
            for (Consumable consumable : v) {
...
//                    Log.d(TAG, String.format("%d remove %s from deliveries", getSerialNumber(), consumable.toString()));
    ...
    private double getLoad() {
        ...
        for (Consumer c : deliveries.keySet())
            for(Consumable i : deliveries.get(c))
                mass += i.getMass();
        return mass;
    }
